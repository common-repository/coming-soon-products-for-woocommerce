...
class Core {
	...
	public function init( $plugin_info ) {
		...
		$this->plugins_list     = array(
			...
			'woo-best-sellers'     => array(
				...
				'thumbnail' => 'woo-best-seller-thumbnail.png',
	...
	public function premium_link( $links ) {
		if ( ! empty( $this->plugin_info['pro_link'] ) ) {
			$links[] = '<a target="_blank" href="' . esc_url_raw( $this->plugin_info['pro_link'] ) . '" ><b style="color:#59c767;">' . esc_html__( 'Premium' ) . '</b></a>';
		}
		return $links;
	}
	...
	public function review_notice( $review_link = '', $is_dismissible = true, $small = false ) {
		...
		?>
		<p style="min-width:650px;" class="mx-auto d-inline-block notice notice-success <?php echo esc_attr( $small ? 'p-2' : 'p-4' ); ?> <?php echo esc_attr( $is_dismissible ? 'is-dismissible' : '' ); ?>">
	...
	public function plugins_sidebar( $exclude = '' ) {
		?>
		<div class="gpls-core-recommended-section my-5">
